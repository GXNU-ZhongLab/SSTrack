...
def generate_attn_box_mask(cfg, bs, device, gt_bbox, box_type='template_box'):
    ...
    if cfg.MODEL.BACKBONE.TEMPLATE_BOX_MASK == 'ALL':
        box_mask_z = None
    elif cfg.MODEL.BACKBONE.TEMPLATE_BOX_MASK == 'CTR_POINT':
    ...
    elif cfg.MODEL.BACKBONE.TEMPLATE_BOX_MASK == 'CTR_REC':
    ...
    elif cfg.MODEL.BACKBONE.TEMPLATE_BOX_MASK == 'GT_BOX':
        if box_type == 'template_box':
        ...
        elif box_type == 'search_box':
            box_mask_z = torch.zeros([bs, search_size, search_size], device=device)
            box_mask_z = generate_bbox_mask(box_mask_z, gt_bbox * search_size).unsqueeze(1).to(torch.float)
        elif box_type == 'search_feat':
            box_mask_z = torch.zeros([bs, search_feat_size, search_feat_size], device=device)
            box_mask_z = generate_bbox_mask(box_mask_z, gt_bbox * search_feat_size).unsqueeze(1).to(torch.float)
