...
from lib.test.analysis.plot_results import generate_formatted_report

import lib.test.analysis.vos_utils as utils
    ...
    for j, sequence in enumerate(dset):
        ...
        for f in frames:
            ...
            if not scores:
                ...
                segmentations[file.name] = imread_indexed(os.path.join(results_path, sequence.dataset, sequence.name, file.name))
            else:
