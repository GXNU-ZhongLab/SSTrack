...
class BaseBackbone(nn.Module):
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        if self.add_cls_token and patch_start_index > 0:
            ...
            # cls_token
            # if cfg.MODEL.BACKBONE.SEARCH_OBJECT_TOKEN:
            #     temporal_tokens = self.cls_token.expand(1, cfg.MODEL.BACKBONE.TOKEN_LEN, -1)
            #     self.temporal_tokens = nn.Parameter(temporal_tokens.clone())
    ...
    def forward_features(self, z, x, track_query=None, token_type="add"):
        ...
        z = self.patch_embed(z)    # (bs*T, (H//16 * W//16), 768)
        ...
        if self.add_cls_token:
            if token_type == "concat":
                ...
                # 方案2: 每次都将track_query与init_token相加
            ...
            # 方案1: 只有第一次使用init_token,后续帧使用track_query
            # query = self.cls_token if track_query is None else track_query # self.cls_token is init query
            # query = query.expand(B, -1, -1)  # copy B times
            # query = query + self.cls_pos_embed
    ...
    def forward_template(self, z, track_query=None, token_type="add"):
        z = torch.stack(z, dim=1)  # [(B, C, H, W),...] to ((B, T, C, H, W))
        B, T, C, H, W = z.shape
        z = z.flatten(0, 1)        # (bs*T, C, H, W)
        z = self.patch_embed(z)    # (bs*T, (H//16 * W//16), 768)
        ...
        #     z = z.view(B, T, -1, z.size()[-1]).contiguous() # (bs, T, HW, C)
        #     z = z.flatten(1, 2)  # (bs, THW, C)
        
        lens_z = z.shape[1]  # HW

        if self.add_cls_token:
            z = torch.cat([query, z], dim=1)     # (B, 1+z, 768)

        z = self.pos_drop(z)

        for i, blk in enumerate(self.blocks):
            z, attn = blk(z, lens_z, return_attention=True)
        
        # new_lens_z = z.shape[1]  # HW
        # z = recover_tokens(z, new_lens_z, mode=self.cat_mode)
    ...
    def forward_search(self, x, track_query=None, token_type="add"):
        B, H, W = x.shape[0], x.shape[2], x.shape[3]
        ...
        for i, blk in enumerate(self.blocks):
            x, attn = blk(x, lens_x, return_attention=False)
    ...
    def forward(self, z, x=None, **kwargs):
        """
        ...
        if (not z is None) and (not x is None):
            x, aux_dict = self.forward_features(z, x, track_query=kwargs['track_query'])
            return x, aux_dict
        
        if x is None:
            x, aux_dict = self.forward_template(z, track_query=kwargs['track_query'])
        else:
            x, aux_dict = self.forward_search(x, track_query=kwargs['track_query'])
