...
class TransformBase:
    ...
    def transform_bbox(self, bbox, image_shape, *rand_params):
        ...
        #### 1/2 of changed coords
        # tl = [ (coord_transf[0,0]+coord_transf[0,3])/2,  (coord_transf[1,0]+coord_transf[1,1])/2]
        # br = [(coord_transf[0,1]+coord_transf[0,2])/2,  (coord_transf[1,2]+coord_transf[1,3])/2]
        # ### 3/4 of changed coords
        # dists=[abs(coord_transf[0, 0] - coord_transf[0, 3]), abs(coord_transf[1,0]-coord_transf[1,1]), abs(coord_transf[0,1]-coord_transf[0,2]), abs(coord_transf[1,2]-coord_transf[1,3])]
        # dist_q = [dist*0.25 for dist in dists]
        # tl = [ (coord_transf[0,0]+coord_transf[0,3])/2 - dist_q[0],  (coord_transf[1,0]+coord_transf[1,1])/2-dist_q[1]]
        # br = [(coord_transf[0,1]+coord_transf[0,2])/2+dist_q[2],  (coord_transf[1,2]+coord_transf[1,3])/2+dist_q[3]]
        # bbox_out = torch.tensor([tl[0], tl[1], br[0]-tl[0], br[1] - tl[1]])
...
class RandomAffine(TransformBase):
    ...
    def roll(self):
        ...
        #print('ar_factor{}'.format(ar_factor))
...
if __name__ == '__main__':
    bb = torch.tensor([[198, 214, 34, 81]])
    im1 = cv.imread('../../1.jpg')
    #im2 = cv.imread('../../0090.jpg')
    #im2 = cv.imread('/home/data/testing_dataset/OTB2015/Bird2/img/0090.jpg')
    im2 = cv.imread('/home/data/testing_dataset/OTB2015/Couple/img/0001.jpg')
    # im2 = cv.imread('/home/data/testing_dataset/OTB2015/BlurCar3/0020.jpg')
    # im2 = cv.imread('/home/data/testing_dataset/OTB2015/BlurCar3/0020.jpg')
    bb0=bb[0]
    #im1 = cv.rectangle(im1, (bb0[0], bb0[1]), (bb0[0] + bb0[2], bb0[1] + bb0[3]), (0, 255, 0), 1)

    flag = True#False#

    if flag: # using tfm
        # tfm_patch = tfm.Transform(tfm.RandomHorizontalFlip(probability=0.5),
        #                                 tfm.RandomAffine(max_rotation=15,max_shear=0.05,max_scale=0.3,max_ar_factor=0),
        #                                 tfm.ToTensorAndJitter(0)
        #                                 #tfm.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) # after toTensor
        #                                  )
        target_patches, bbs, pad_sz = crop_target_patch(im1, bb, pad_ratio=(0.3, 0.3)) # 0.3,0.3
        cv.imshow('1', target_patches)
        cv.weitKey(0)
        cv.destroyWindow()
        h,w = target_patches.shape[0:2]
        #bb[0] = torch.tensor([ bbs[0], bbs[1], bbs[2], bbs[3]])
        #bb[0] = [bbs[0], bbs[1], bbs[2], bbs[3]]
        bbs = bbs*1.0
        target_patches, bbs = tfm_patch(image=target_patches,bbox=bbs.view(1,-1))
    else:
        # input image should be PIL or Tensor
        transform_patch = transforms.Compose([
            #transforms.RandomApply(transforms.RandomAffine(degrees= 15, shear=(-10,10,-10,10)), p=0.6),
            transforms.ToTensor(),
            transforms.ToPILImage(),
            transforms.RandomPerspective(distortion_scale=0.5),
            transforms.ToTensor(),
            # transforms.RandomResizedCrop(224, scale=(0.2, 1.)), #
            #transforms.RandomHorizontalFlip(), # default p =0.5, image should be [...,H,W]
        ])
        #
        # tranform_image = transforms.Compose(
        #     [transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=0.8),
        #      transforms.RandomGrayscale(p=0.2),  # probability, image should be [...,3,H,W]
        #      transforms.RandomApply([GaussianBlur([.1, 2.])], p=0.5),  # ker_sz, probability
        #      transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # tensor image
        #
        #      ])

        target_patches, bb, pad_sz = crop_target_patch(im1, bb, pad_ratio=(0.2, 0.2))
        #h, w = target_patches.shape[0:2]
        bbs = torch.tensor([[bb[0],bb[1],bb[2],bb[3]]])
        target_patches = transform_patch(target_patches)

    #target_patches = transform_patch(target_patches)
    target_patches = (target_patches*255).int()
    im, bb = patch_syn( target_patches.permute(1,2,0), bbs[0], im2)
    im_bb = cv.rectangle(im,(bb[0],bb[1]),(bb[0]+bb[2],bb[1]+bb[3]), (0,0,255),1)
    cv.imshow('1', im_bb)
    cv.weitKey(0)
    cv.destroyWindow()
    print('1')