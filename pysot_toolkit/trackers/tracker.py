...
class Tracker(object):
    ...
    def track(self, image, info: dict = None, dataset=None, video=None, frame=0) -> dict:
        ...
        # ------------------ visualize heatmap -------------------
        from visual_tools.draw_heatMap import draw_heatMap
        # pred point
        row = best_idx // 32
        column = best_idx - row * 32
        pred_point = (row * 8, column * 8)
        # pred bbox
        pred_box = (torch.tensor(bbox)[np.newaxis, :].mean(dim=0) * 256)
        # draw_heatMap(pscore, x_crop, pred_box=pred_box, pred_point=pred_point, dataset=dataset, video=video, frame=frame)
        draw_heatMap(pscore, x_crop, dataset=dataset, video=video, frame=frame)
        # ------------------ visualize heatmap -------------------
